...
SDK PHP para la API de Contabo v1, construido desde el OpenAPI oficial de `contabo/cntb` y envuelto con una capa manual para OAuth2, `x-request-id` automùtico y uso opcional en Laravel.
...
El wrapper agrega `Authorization: Bearer ...` y `x-request-id` automùticamente. Si Contabo responde `401`, el SDK solicita un token nuevo y reintenta una vez.

## Uso Laravel

Publica la configuraciùn:
...
La documentaciùn pùblica de Contabo se encuentra en [api.contabo.com](https://api.contabo.com/). La especificaciùn vendoreada se sincroniza desde:
